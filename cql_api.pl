...
%       Map an AccessToken back to a UserId for auditing purposes
access_token_to_user_id(X, X).

%%      log_selects.
%       Selects will be logged via advise/4 if this succeeds.
log_selects:- fail.
...
dbms_normalize_name(_, Name, Name).


%%      default_schema(-Schema).
%       Schema to apply when none is specified explicity
default_schema(seattle).

%%      max_db_connections(+Count).
%       Limit each thread to Count connections.
max_db_connections(10).

%%      statistic_monitored_attribute(+Schema, +TableName, +ColumnName).
statistic_monitored_attribute(_, _, _):- fail.
...
schema_fact(Schema, 'PostgreSQL', Fact):-
        ...
        ( Fact = user:database_attribute(table, Schema, EntityName, ColumnName, DomainOrNativeType, AllowsNulls, IsIdentity, ColumnDefault)
        ; Fact = user:odbc_data_type(Schema, EntityName, ColumnName, Mapped)
        ).

schema_fact(Schema, 'PostgreSQL', user:primary_key_column_name(Schema, EntityName, ColumnName)):-
...
schema_fact(Schema, 'PostgreSQL', user:database_domain(DomainName, Mapped)):-
...
schema_fact(Schema, 'PostgreSQL', user:routine_return_type(Schema, RoutineName, Mapped)):-
...
schema_fact(Schema, 'SQLite', Fact):-
        ...
        ( Fact = user:database_attribute(table, Schema, EntityName, ColumnName, native_type(ColumnType), AllowsNulls, IsIdentity, ColumnDefault)
        ; Fact = user:odbc_data_type(Schema, EntityName, ColumnName, OdbcDataType)
        ; IsPkMember == 1, Fact = user:primary_key_column_name(Schema, EntityName, ColumnName)
        ).
...
user:term_expansion(:-build_schema(Schema), Facts):-
        ...
        findall(Fact, schema_fact(Schema, DBMS, Fact), Facts).